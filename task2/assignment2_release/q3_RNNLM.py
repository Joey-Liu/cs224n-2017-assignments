...
class RNNLM_Model(LanguageModel):
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("projection"):
      ...
      b_2 = tf.get_variable("b_2", (len(self.vocab)), initializer=tf.constant_initializer(0.0))

    outputs = []
    for out in rnn_outputs:
      outDrop = tf.nn.dropout(out, self.dropout_placeholder)
  ...
  def add_model(self, inputs):
    ...
    # 进行序列预测,注意,预测时不对第一个单词进行预测
    state = self.initial_state
    rnn_outputs = []
    for step in xrange(self.config.num_steps):
      words = inputs[step]
      act = tf.matmul(tf.nn.dropout(state, self.dropout_placeholder), H)\
            + tf.matmul(tf.nn.dropout(words, self.dropout_placeholder), I) \
            + b_1
      state = tf.sigmoid(act)
...
def test_RNNLM():
  ...
  with tf.variable_scope('RNNLM') as scope:
    ...
    # 我不知道这里为什么错了, 在创建gen_model对象时,最后的add_train_op中,adam2变量创建失败,
    # adam2变量名为 RNNLM/RNNLM/embedding/......,不知道为什么会产生这个错误
