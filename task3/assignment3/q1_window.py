...
class Config:
    ...
    """
    n_word_features = 2 # Number of features for every word in the input. for this problem are word vector and word casing
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        startFeatures = window_size * [start]
        endFeatures = window_size * [end]

        startFeatures.extend(sentence)
        startFeatures.extend(endFeatures)
        sentence = startFeatures
        for i in range(window_size, len(sentence) - window_size):
            features = sentence[i - window_size : i + window_size + 1]
            tmp = [];
            for fea in features:
                tmp.extend(fea)
            windowed_data.append((tmp, labels[i - window_size]))
    ### END YOUR CODE
