...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.lables_placeholder = tf.placeholder(tf.float32, [None, self.config.n_classes], name="labels")
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embedding_matrix = tf.Variable(self.pretrained_embeddings, 'embedding')
        features = tf.nn.embedding_lookup(embedding_matrix, self.input_placeholder)
        embeddings = tf.reshape(features, [-1, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("NN"):
            W = tf.get_variable("W", shape=[self.config.n_features * self.config.embed_size, self.config.hidden_size],
            ...
            b2 = tf.get_variable("b2", shape=[self.config.n_classes, ], initializer=tf.constant_initializer(0.0))
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        loss = tf.nn.softmax_cross_entropy_with_logits(labels=self.lables_placeholder, logits=pred)
