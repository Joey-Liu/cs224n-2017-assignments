...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      params = tf.get_variable(
        ...
        tf.float32,
        xavier_weight_init())
      # params.shape:(10000, 50)
      # self.input_placeholder: (?, 10)
      # lookup.shape: (?, 10, 50)
      ...
      ## 这个地方squeeze 需要注明axis=1, 因为是placeholder, 有一维未知
      # inputs: list, 10 elements,  each element's shape is [?, 50]
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("projection"):
      U = tf.get_variable(name="U", shape=[self.config.hidden_size, len(self.vocab)],
                          dtype=tf.float32)
      b_2 = tf.get_variable(name="b_2", shape=[len(self.vocab), ], dtype=tf.float32,
                            initializer=tf.constant_initializer(0.0))
    outputs = []
    for rop in rnn_outputs:
      # outDrop = tf.nn.dropout(rop, self.dropout_placeholder)
      # outputs.append(tf.matmul(outDrop, U) + b_2)
      outputs.append(tf.matmul(rop, U) + b_2)
    ### END YOUR CODE
    return outputs
  ...
  def add_model(self, inputs):
    ...
    self.initial_state = tf.zeros(shape=[self.config.batch_size, self.config.hidden_size],
                                  dtype=tf.float32)
    self.final_state = tf.zeros(shape=[self.config.batch_size, self.config.hidden_size],
                                  dtype=tf.float32)
    ...
    for words in inputs:
      ...
      state = tf.sigmoid(tf.matmul(state, H) + tf.matmul(words, I) + b_1)
