...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embed = tf.Variable(self.pretrained_embeddings, name="embed")
        features = tf.nn.embedding_lookup(embed, self.input_placeholder)
        embeddings = tf.reshape(features, shape=(-1, self.max_length, self.config.n_features*self.config.embed_size))
        ### END YOUR CODE
        return embeddings
