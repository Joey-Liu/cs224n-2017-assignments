...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.name_scope("embedding"):
        ...
        window = tf.reshape(tensor=window,shape=[-1, Config.window_size * Config.embed_size])
      ### END YOUR CODE
      return window
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer"):
      W = tf.get_variable("W", shape=[Config.window_size*Config.embed_size, Config.hidden_size]
                          , dtype=tf.float32, initializer=xavier_weight_init())
      b1 = tf.get_variable("b1", shape=[Config.hidden_size, ], dtype=tf.float32
                          , initializer=tf.constant_initializer(0.0))
    with tf.variable_scope("hidden_layer"):
      U = tf.get_variable("U", shape=[Config.hidden_size, Config.label_size],
                          dtype=tf.float32, initializer=xavier_weight_init())
      b2 = tf.get_variable("b2", shape=[Config.label_size,], dtype=tf.float32,
                           initializer=tf.constant_initializer(0.0))

    z2 = tf.matmul(window, W) + b1
    a2 = tf.nn.relu(z2)
    a2 = tf.nn.dropout(a2, keep_prob=self.dropout_placeholder)

    z3 = tf.matmul(a2, U) + b2
    z3 = tf.nn.dropout(z3, keep_prob=self.dropout_placeholder)
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    l1_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder
                                                      , logits=y))
    l2_loss = tf.get_collection(key="l2_loss")[0] * self.config.l2
