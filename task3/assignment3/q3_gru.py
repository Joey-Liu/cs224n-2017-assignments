...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self): 
        ...
        _, preds = tf.nn.dynamic_rnn(cell, x, time_major=False, dtype=tf.float32)
        preds = tf.sigmoid(preds)
        # preds = tf.reshape(preds, shape=[-1, 1])
        ### END YOUR CODE
    ...
    def add_training_op(self, loss):
        ...
        gradients = [tmp[0] for tmp in gradients_vars]
        vars = [tmp[1] for tmp in gradients_vars]

        # self.grad_norm = tf.global_norm(gradients)
        if self.config.clip_gradients:
            gradients, self.grad_norm = tf.clip_by_global_norm(t_list=gradients, clip_norm=self.config.max_grad_norm)
        self.grad_norm = tf.global_norm(gradients)

        new_grads_vars = zip(gradients, vars)
        train_op = optimizer.apply_gradients(new_grads_vars)
        ### END YOUR CODE
