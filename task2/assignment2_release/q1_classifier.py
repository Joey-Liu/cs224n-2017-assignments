...
class SoftmaxModel(Model):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    self.input_placeholder = tf.placeholder(shape=[None, self.config.n_features], dtype=tf.float32)
...
#    b = tf.get_variable(name="b", shape=[self.config.n_classes, 1], \
#                        initializer=tf.random_normal_initializer(0, 0.1))
#    affine_transform = tf.matmul(W, input_data) + b
#    out = softmax(affine_transform)
    ...
    affine_transform = tf.matmul(input_data, W) + b
    out = softmax(affine_transform)
    ### END YOUR CODE
    return out
  ...
  def add_loss_op(self, pred):
    ...
    """
    ### YOUR CODE HERE
#    labels = np.zeros([self.config.n_samples, self.config.n_classes])
#    for i in range(self.config.n_samples):
#        labels[i, self.input_labels[i]] = 1
#    
