...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        masks = [True] * sentence_length
        if sentence_length < max_length:
            extra_num = max_length - sentence_length
            # sentence.extend([zero_vector] * extra_num)
            # labels.extend([zero_label] * extra_num)
            masks.extend([False] * extra_num)
            new_sentence = sentence + [zero_vector] * extra_num
            new_labels = labels + [zero_label] * extra_num
        ret.append([new_sentence[:max_length], new_labels[:max_length], masks[:max_length]])
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embedding_matrix = self.pretrained_embeddings
        embeddings = tf.nn.embedding_lookup(embedding_matrix, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        b_2 = tf.get_variable("b_2", shape=(self.config.n_classes), dtype=tf.float32,
                              initializer=tf.constant_initializer(0.0))
        # h = tf.zeros(shape=(x.shape[0], Config.hidden_size))
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_drop_t = tf.nn.dropout(h, self.dropout_placeholder)
