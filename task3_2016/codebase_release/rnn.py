...
#                            initializer=tf.contrib.layers.xavier_initializer())
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ...
            curr_node_tensor = tf.expand_dims(tf.gather(embedding, word_index), 0)
            ### END YOUR CODE
        else:
            node_tensors.update(self.add_model(node.left))
            node_tensors.update(self.add_model(node.right))
            ### YOUR CODE HERE
            left_node = node_tensors[node.left]
            right_node = node_tensors[node.right]
            z2 = tf.matmul(tf.concat(1, [left_node, right_node]), W1) + b1
            curr_node_tensor = tf.nn.relu(z2)
            ### END YOUR CODE
    ...
    def add_projections(self, node_tensors):
        ...
        with tf.variable_scope("Projection", reuse=True):
            ...
            z3 = tf.matmul(node_tensors, U) + bs
            logits = tf.nn.softmax(z3)
        ### END YOUR CODE
        return logits
    ...
    def loss(self, logits, labels):
        ...
        loss = l2_loss * self.config.l2 + ce_loss
        # END YOUR CODE
        return loss
        ...
        while step < len(self.train_data):
            with tf.Graph().as_default(), tf.Session() as sess:
                ...
                if new_model:
                    ...
                    print "This model initialize again..."
                    sess.run(init)
                else:
                    saver = tf.train.Saver()
