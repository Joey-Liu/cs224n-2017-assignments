...
def softmax(x):
  ...
  ### YOUR CODE HERE
  xm = tf.reduce_max(x, keep_dims=True, reduction_indices=1)
  xt = x - xm
  xtexp = tf.exp(xt)
  out = xtexp / tf.reduce_sum(xtexp, reduction_indices=1, keep_dims=True)
  ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
  ...
  """
  ### YOUR CODE HERE
  te = tf.to_float(y) * yhat
  ys = tf.reduce_sum(te, reduction_indices=1)
  out = -tf.log(ys)
